...
import { formdataToJSON, clamp } from "./utils.js";
...
export class TestEditor {
  ...
  createAnswerItemEl(answer, question, questionIdx, idx) {
    ...
    editButtonEl.classList.add(
      ...
      "editor-utility__button-edit",
      "button"
    );
    editButtonEl.type = "menu";
    ...
    editButtonEl.addEventListener("click", () => alert("edit!"));
    ...
    editorUtilityEl.append(editButtonEl, deleteButtonEl);
  ...
  createBody() {
    ...
    this.editorFormEl.action = "/api/tests/index.php";
    this.editorFormEl.method = "POST";
    ...
    this.editorFormSaveButtonEl.addEventListener("click", (e) => {
      e.preventDefault();
      alert("save");
      console.log(new FormData(this.editorFormEl));
      const jsonData = JSON.parse(
        formdataToJSON(new FormData(this.editorFormEl))
      );
...
async function init() {
  ...
  console.log(testId, testId ? "true" : "false");
